...
%%
% determine node to node connectivities
% 
% usage:
% conn=nodeconn(elems[,iblanks]);

function conn=nodeconn(conn,iblanks)

nnel=size(conn,2);
[n,i]=sort(reshape(conn',numel(conn),1));
i=ceil(i/nnel);
node=unique(n);
conn=reshape(conn(i,:)',numel(i)*nnel,1);
n=reshape((n*ones(1,nnel))',numel(n)*nnel,1);
i=n~=conn;
n=n(i);
conn=conn(i);
